...
namespace JsCore
{
	...
	JSStringRef	GetString(JSContextRef Context,std::string_view Value);
	...
	JSValueRef	GetValue(JSContextRef Context,std::string_view Value);
	...
	//	JSON to object
	TObject		ParseObjectString(JSContextRef Context,std::string_view JsonString);
	...
	void		ThrowException(JSContextRef Context, JSValueRef ExceptionHandle, std::string_view ThrowContext);
...
{
public:
	TInstance(std::string_view RootDirectory,std::string_view ScriptFilename,std::function<void(int32_t)> OnShutdown);
	~TInstance();

	std::shared_ptr<JsCore::TContext>	CreateContext(std::string_view Name);
	...
	void								LoadModule(const std::string& ModuleFilename,std::function<void(TLocalContext&,TObject&)> OnLoadModule,std::function<void(std::string_view)> OnError);
...
public:
	...
	virtual void			Return(std::string_view Value) bind_override	{	mReturn = GetValue( GetContextRef(), Value );	}
	...
	virtual void			SetArgumentString(size_t Index,std::string_view Value) bind_override;
...
public:
	TTemplate()	{}
	TTemplate(TContext& Context,std::string_view Name) :
		mName		( Name ),
		mContext	( &Context )
	{
	}
	
	bool			operator==(std::string_view Name) const	{	return mName == Name;	}
	...
	void			RegisterClassWithContext(TLocalContext& Context,std::string_view ParentObjectName,std::string_view OverrideLeafName);
...
public:
	...
	virtual bool			HasMember(std::string_view MemberName) bind_override;
	bool					IsMemberArray(std::string_view MemberName);
	bool					IsMemberNull(std::string_view MemberName);

	virtual JsCore::TObject	GetObject(std::string_view MemberName) bind_override;
	virtual std::string		GetString(std::string_view MemberName) bind_override;
	virtual uint32_t		GetInt(std::string_view MemberName) bind_override;
	virtual float			GetFloat(std::string_view MemberName) bind_override;
	virtual JsCore::TFunction	GetFunction(std::string_view MemberName) bind_override;
	virtual bool			GetBool(std::string_view MemberName) bind_override;
	template<typename TYPE>
	void					GetArray(std::string_view MemberName,ArrayBridge<TYPE>&& Values)
	{
		auto Member = GetMember(MemberName);
		JsCore::EnumArray(mContext, Member,Values);
	}

	void			SetObject(std::string_view Name, const JsCore::TObject& Object) bind_override;
	void			SetObjectFromString(std::string_view Name, std::string_view JsonString) bind_override;
	void			SetFunction(std::string_view Name,JsCore::TFunction& Function) bind_override;
	inline void		SetFloat(std::string_view Name,float Value)	{	SetMember(Name, GetValue(mContext, Value));	}
	inline void		SetString(std::string_view Name,std::string_view Value)	{	SetMember(Name, GetValue(mContext, Value));	}
	inline void		SetBool(std::string_view Name,bool Value)		{	SetMember(Name, GetValue(mContext, Value));	}
	template<typename INT>
	inline void		SetInt(std::string_view Name, INT Value)	{	SetMember(Name, GetValue(mContext, Value));	}
	void			SetNull(std::string_view Name) bind_override;
	void			SetUndefined(std::string_view Name) bind_override;
	void			SetArray(std::string_view Name,JsCore::TArray& Array) bind_override;
	template<typename TYPE>
	inline void				SetArray(std::string_view Name,const ArrayBridge<TYPE>&& Values) bind_override
	{
	...
	//	Jscore specific
private:
	JSValueRef		GetMember(std::string_view MemberName);
	void			SetMember(std::string_view Name,JSValueRef Value);
...
public:
	...
	TPersistent(Bind::TLocalContext& Context,const TObject& Object,std::string_view DebugName)	{	Retain( Context, Object, DebugName );	}
	TPersistent(Bind::TLocalContext& Context,const TFunction& Object,std::string_view DebugName)	{	Retain( Context, Object, DebugName );	}
...
private:
	void		Retain(Bind::TLocalContext& Context,const TObject& Object,std::string_view DebugName);
	void		Retain(Bind::TLocalContext& Context,const TFunction& Object,std::string_view DebugName);
	void		Retain(const TPersistent& That);
	void 		Release();
	
	static void	Release(JSGlobalContextRef Context,JSObjectRef ObjectOrFunc,std::string_view DebugName);
	static void	Retain(JSGlobalContextRef Context,JSObjectRef ObjectOrFunc,std::string_view DebugName);
...
public:
	TContext(TInstance& Instance,JSGlobalContextRef Context,std::string_view Filename);
	~TContext();

	//	load module should probably be in instance now, as it just loads a context	
	virtual void			LoadModule(std::string_view Filename,std::function<void(TLocalContext&,TObject&)> OnLoadModule,std::function<void(std::string_view)> OnError);
	
	virtual void			LoadScript(std::string Source,const std::string& Filename,JSObjectRef Global=JSObjectRef(nullptr)) bind_override;
	virtual void			LoadScript(const std::string& Source,std::string_view Filename,JsCore::TObject Global) bind_override;
	...
	template<const char* FunctionName>
	void					BindGlobalFunction(std::function<void(JsCore::TCallback&)> Function,std::string_view ParentName=std::string());
	
	JsCore::TObject			GetGlobalObject(TLocalContext& LocalContext,std::string_view ObjectName=std::string());	//	get an object by it's name. empty string = global/root object
	virtual void			CreateGlobalObjectInstance(const std::string& ObjectType,const std::string& Name) bind_override;
	virtual JsCore::TObject	CreateObjectInstance(TLocalContext& LocalContext,std::string_view ObjectTypeName=std::string());
	JsCore::TObject			CreateObjectInstance(TLocalContext& LocalContext,std::string_view ObjectTypeName,ArrayBridge<JSValueRef>&& ConstructorArguments);
	void					ConstructObject(TLocalContext& LocalContext,std::string_view ObjectTypeName,JSObjectRef NewObject,ArrayBridge<JSValueRef>&& ConstructorArguments);

	virtual JsCore::TPromise	CreatePromise(Bind::TLocalContext& LocalContext, std::string_view DebugName) bind_override;
	virtual std::shared_ptr<JsCore::TPromise>	CreatePromisePtr(Bind::TLocalContext& LocalContext, std::string_view DebugName) bind_override;
	
	template<typename OBJECTWRAPPERTYPE>
	void				BindObjectType(std::string_view ParentName=std::string(),std::string_view OverrideLeafName=std::string());
	...
	JSValueRef			CallFunc(TLocalContext& LocalContext,std::function<void(JsCore::TCallback&)> Function,JSObjectRef This,size_t ArgumentCount,const JSValueRef Arguments[],JSValueRef& Exception,std::string_view FunctionContext);
...
public:
	TPromise()	{}
	TPromise(Bind::TLocalContext& Context,TObject& Promise,TFunction& Resolve,TFunction& Reject,std::string_view DebugName);
	~TPromise();
	
	//	const for lambda[=] copy capture
	void			Resolve(Bind::TLocalContext& Context,std::string_view Value) const		{	Resolve( Context, GetValue( Context.mLocalContext, Value ) );	}
	...
	void			Reject(Bind::TLocalContext& Context,std::string_view Value) const			{	Reject( Context, GetValue( Context.mLocalContext, Value ) );	}
...
template<const char* FUNCTIONNAME>
inline void JsCore::TContext::BindGlobalFunction(std::function<void(JsCore::TCallback&)> Function,std::string_view ParentName)
{
	auto FunctionPointer = GetRawFunction<FUNCTIONNAME>( Function );
	BindRawFunction( FUNCTIONNAME, std::string(ParentName), FunctionPointer );
}
...
template<typename OBJECTWRAPPERTYPE>
inline void JsCore::TContext::BindObjectType(std::string_view ParentName,std::string_view OverrideLeafName)
{
