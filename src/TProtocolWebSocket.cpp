...
template<> bool TSerialisation<THex<uint32>>::ExportText(std::stringstream& String) const
{
	int TypeNibbles = 2*sizeof(mValue.mValue);
	for ( int i=0;	i<TypeNibbles;	i++ )
	{
		char hex = mValue.mValue >> ((TypeNibbles-1-i)*4);
		hex &= (1<<4)-1;
		if ( hex >= 10 )
			hex += 'A' - 10;
		else
...
std::string TWebSocketMessageHeader::GetMaskKeyString() const
{
	...
	TSerialisation<THex<uint32>> Serial( Mask32 );
	std::stringstream s;
	Serial.ExportText( s );
	return s.str();
}
